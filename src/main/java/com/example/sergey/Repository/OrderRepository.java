...
public interface OrderRepository extends JpaRepository<Order,Long> {
	...
	@Query(value="select * from orderlist where lower(author) like lower(concat('%',:avtor,'%'))"
			...
			+ " and (lower(cart) like concat('%',:filter3,'%') or lower(cart) like concat('%',:filter4,'%',:filter5,'%') or lower(cart) like concat('%',:filter5,'%',:filter4,'%')) "
			+ " and lower(comment) similar to concat('%',:comment,'%') order by ordernumber",nativeQuery=true)
